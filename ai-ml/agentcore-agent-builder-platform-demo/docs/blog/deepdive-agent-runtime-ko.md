...
>
> *Part 1을 읽지 않았어도 따라올 수 있도록 썼지만, "왜 개발이 아니라 조립인가"의 맥락은 [Part 1](./part1-draft-ko.md)에 있습니다. 코드를 직접 보고 싶다면 `code/agent-runtime/agent_runner.py`와 함께 읽으면 가장 좋습니다.*
...
## 다시, 조립으로 — Part 1과 이어 붙이기

Part 1에서 Builder가 만들어 카드로 등록한 세 부품 — Context Boundary, Gateway, Delegation — 은 여기서 각각 제 자리를 찾습니다. **Context Boundary**는 persona-injection 훅이 시스템 프롬프트에 강제하고, **Gateway**는 MCP 커넥터가 SigV4로 바인딩하며, **Delegation**은 `delegate_*` 도구와 depth 가드가 떠받칩니다. UI에서 고른 설정 한 건이 실행 가능한 에이전트로 살아나는 모든 접점이 이 400줄 안에 있는 셈입니다.

그리고 Part 1의 [시리즈 노트]에서 예고했듯, 이 글이 해부한 자체 구현 런타임 — 특히 Harness(범위·깊이 검증)와 관측성 계층 — 은 AgentCore에 추가되는 **관리형 기능으로 상당 부분 대체**할 수 있습니다. "직접 짠 안전핀"이 "플랫폼이 보장하는 기본 동작"으로 옮겨갈 때 무엇이 줄고 무엇이 남는지는 다음 편에서 before/after로 다룹니다.
