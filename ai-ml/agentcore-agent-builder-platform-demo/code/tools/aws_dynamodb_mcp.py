...
def lambda_handler(event, context):
    ...
    if not t:
        if "table_name" in args and "key" in args: t = "get_item"
        elif "table_name" in args and "query" in str(args).lower(): t = "query_table"
        elif "table_name" in args: t = "describe_table"
        else: t = "list_tables"

    try:
        ddb = get_client('dynamodb', region, role_arn)
        ddb_r = get_resource('dynamodb', region, role_arn)
