...
def call_mcp_tool(tool_name, arguments):
    ...
    with urllib.request.urlopen(req, timeout=30) as resp:  # nosec B310 - fixed https MCP_URL constant, not user-controlled
...
def lambda_handler(event, context):
    ...
    arguments = params.get("arguments", params) if "arguments" in params else params
    ...
    if not tool_name:
        if "search_phrase" in arguments:
            tool_name = "search_documentation"
        elif "url" in arguments and "max_length" in arguments:
            tool_name = "read_documentation"
        elif "url" in arguments:
            tool_name = "recommend"
        elif "resource_type" in arguments:
