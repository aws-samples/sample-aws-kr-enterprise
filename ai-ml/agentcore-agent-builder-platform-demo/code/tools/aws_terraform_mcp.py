...
def search_registry_module(module_url, version=None):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:  # nosec B310 - api_url is built from the fixed https://registry.terraform.io literal, scheme statically constrained
            data = json.loads(resp.read().decode())
        result = {
            "name": data.get("name", ""),
...
def lambda_handler(event, context):
    ...
    if not t:
        if "asset_name" in args and "awscc" in args.get("asset_name", ""):
            t = "SearchAwsccProviderDocs"
        elif "asset_name" in args:
            t = "SearchAwsProviderDocs"
        elif "module_url" in args:
