...
def lambda_handler(event, context):
    ...
    if not t:
        ...
        elif "service_name" in args and "cluster" in args:
    ...
    try:
        ...
        if t == "ecs_resource_management":
            return handle_resource(args, role_arn)
        elif t == "ecs_troubleshooting_tool":
            return handle_troubleshoot(args, role_arn)
        elif t == "wait_for_service_ready":
            return handle_wait_service(args, role_arn)
...
def handle_resource(args, role_arn=None):
    ecs = get_client('ecs', 'ap-northeast-2', role_arn)
    ...
    # List ECR repositories / ECR 리포지토리 목록 조회
        ...
        ecr = get_client('ecr', 'ap-northeast-2', role_arn)
...
def handle_troubleshoot(args, role_arn=None):
...
def handle_wait_service(args, role_arn=None):
    ecs = get_client('ecs', 'ap-northeast-2', role_arn)
    cluster = args.get("cluster", "")
