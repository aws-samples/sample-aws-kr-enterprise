...
def lambda_handler(event, context):
    ...
    if not t:
        if "query" in args and "event_data_store" in args: t = "lake_query"
        elif "query_id" in args and "status" in str(args).lower(): t = "get_query_status"
        elif "query_id" in args: t = "get_query_results"
        elif "event_data_store" in str(args).lower() or "data_store" in str(args).lower(): t = "list_event_data_stores"
    ...
    try:
        ...
        if t == "lookup_events":
            ...
            # Build lookup attributes (API supports only 1) / 조회 속성 구성 (API는 1개만 지원)
            lookup_attrs = []
            ...
            # Query CloudTrail management events / CloudTrail 관리 이벤트 조회
        ...
        # List CloudTrail Lake event data stores / CloudTrail Lake 이벤트 데이터 스토어 목록 조회
        ...
        # Check CloudTrail Lake query execution status / CloudTrail Lake 쿼리 실행 상태 확인
        ...
        # Get CloudTrail Lake query results / CloudTrail Lake 쿼리 결과 조회
