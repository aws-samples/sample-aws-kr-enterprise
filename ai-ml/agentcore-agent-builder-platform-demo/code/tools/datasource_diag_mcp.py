...
def lambda_handler(event, context):
    ...
    if not t:
        if "url" in args and "datasource_type" in args:
            ...
            t = "resolve_dns"
        elif "nlb_dns" in args or "nlb_arn" in args:
            t = "check_nlb_targets"
        elif "source_cidr" in args:
            t = "analyze_security_groups"
        elif "source_vpc_id" in args and "destination_vpc_id" in args:
            t = "trace_network_path"
        elif "url" in args:
            t = "test_http_connectivity"
        elif "cluster_name" in args and "service_name" in args:
...
def _validate_datasource_url(args):
    ...
    ssrf_risk = "none"
    blocked_patterns = ["169.254.169.254", "metadata.google", "localhost", "127.0.0.1", "::1", "0.0.0.0"]  # nosec B104 - SSRF blocklist entry, not an interface bind
...
def _test_http_connectivity(args):
    ...
    if urlparse(url).scheme not in ("http", "https"):
        return err("Only http/https URLs are supported")

    start = time.time()
    try:
        req = Request(url, headers=headers or {})
        req.add_header("User-Agent", "AWSops-Diagnostics/1.0")
        resp = urlopen(req, timeout=timeout)  # nosec B310 - scheme validated to http/https above
...
def _check_k8s_service_endpoints(args, region, role_arn):
    ...
    try:
        ...
        resp = urlopen(req, timeout=10, context=ctx)  # nosec B310 - endpoint is the https EKS cluster API server URL from describe_cluster, not user-controlled
