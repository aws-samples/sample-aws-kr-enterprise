#!/usr/bin/env python3
"""Register Lambda→Gateway targets for all 8 Gateways.
Usage: AWS_REGION=ap-northeast-2 ACCOUNT_ID=xxx python3 register-gateway-targets.py

Excludes: istio-mcp-target, steampipe-query-target
Includes: finops-mcp-target, datasource-diag-target
"""
import boto3
import os
import sys
...
if not REGION:
    print("ERROR: AWS_REGION is not set. Export AWS_REGION first.", file=sys.stderr)
    sys.exit(1)
...
def find_gateway(name_pattern):
    """Find gateway by exact name match"""
...
if not gw:
    print('WARNING: Network gateway not found — skipping'); gw = None
...
if not gw:
    print('WARNING: Container gateway not found — skipping'); gw = None
